...
class SlidingWindowRateLimiter:
    ...
    def _cleanup_window(self, user_id: str, current_time: float) -> None:
        """Видаляє застарілі таймстемпи з вікна користувача."""
    ...
    def can_send_message(self, user_id: str) -> bool:
        ...
        window = self.user_history.get(user_id, deque())
    ...
    def record_message(self, user_id: str) -> bool:
        ...
        current_time = time.time()
        self._cleanup_window(user_id, current_time)
        window = self.user_history.get(user_id)
        if window is None:
            window = deque()
            self.user_history[user_id] = window
    ...
    def time_until_next_allowed(self, user_id: str) -> float:
        """Повертає час у секундах до наступного дозволеного повідомлення."""
        current_time = time.time()
        self._cleanup_window(user_id, current_time)
        window = self.user_history.get(user_id)
